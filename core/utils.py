...
notImgSvg = """
<svg width="800px" height="800px" viewBox="0 0 24 24" fill="none" class="imgAdmin">
...
"""
def drawImg(name, image):
  imgDraw = '<img src={url} class="imgAdmin" alt={name} />'
  if image:
    return mark_safe(imgDraw.format(url=image.url, name=name))
  else:
    return mark_safe(notImgSvg)
